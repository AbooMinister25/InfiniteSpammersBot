...
class UtilsCog(commands.Cog):
    ...
    async def store(ctx, title, data):
        ...
        embed = discord.Embed(title=":white_check_mark: Data stored succesfully", description="You can call this data using the !call or !retrieve commands", color=discord.Colour.green())
        await ctx.channel.send(embed=embed)
    ...
    async def call(ctx, keyword: str):
        ...
        try:
            data = data[keyword]
            embed = discord.Embed(title=":white_check_mark: Data Retrieved Succesfully", description=f"Data: {data}", color=discord.Colour.green())
        except:
            embed = discord.Embed(title=":x: Data Retrieval Failed", description=f"Invalid keyword {keyword} given", color=discord.Colour.red())
        await ctx.channel.send(embed=embed)
